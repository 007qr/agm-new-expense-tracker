...
import { WarehouseTransaction, EntityWarehouse, EntityVariantWarehouse, Destination } from '~/drizzle/schema'; // Adjust path if needed
    ...
    return (
        <div class="w-full flex items-center justify-center p-6 bg-brand min-h-[85vh] font-sans text-[#fff]">
            <div class="w-full max-w-4xl animate-in fade-in zoom-in-95 duration-500">
                {/* Header */}
                <div class="mb-10">
                    <h1 class="text-2xl font-semibold text-[#fff]">Record Movement</h1>
...
function TransactionFormContent() {
    ...
    const [transactionType, setTransactionType] = createSignal<'credit' | 'debit'>('credit');

    const availableVariants = createMemo(() => {
        return variants().filter((v) => v.entity_id === selectedEntityId());
    });

    const selectedUnit = createMemo(() => {
        return entities().find((e) => e.id === selectedEntityId())?.unit || 'Units';
    });
    ...
    return (
        ...
        >
            ...
            {/* --- SECTION 1: TYPE & ITEM --- */}
            <div class="space-y-5">
                <h2 class="text-xs font-semibold text-[#fff]/50 uppercase tracking-wide mb-4">Transaction Details</h2>
