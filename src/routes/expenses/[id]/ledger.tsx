            ...
            <div class="mb-8 flex justify-between items-start">
                <div>
                    ...
                    <p class="text-base text-zinc-600">
                        For site: <span class="font-medium text-zinc-900">{data()?.destination}</span>
                    </p>
                </div>
                    ...
                    <table class="w-full text-left border-collapse">
                        ...
                        <thead>
                            <tr class="border-b border-zinc-200">
                                ...
                                <th class="py-5 px-4 text-xs font-bold uppercase tracking-wider text-zinc-800">
                                    From/To
                                </th>
                                <th class="py-5 px-4 text-right text-xs font-bold uppercase tracking-wider text-zinc-800">
                                    Rate (₹)
                                </th>
                                <th class="py-5 px-4 text-right text-xs font-bold uppercase tracking-wider text-zinc-800">
                                    Quantity
                                ...
                                <th class="py-5 pr-8 text-right text-xs font-bold uppercase tracking-wider text-zinc-800">
                                    Actions
                                </th>
                            </tr>
                        </thead>
                                    ...
                                    <For each={data()?.transactions}>
                                        {(tx) => (
                                            <tr class="group hover:bg-zinc-50">
                                                <td class="py-4 px-4 text-sm text-zinc-700">
                                                    {new Date(tx.created_at).toLocaleDateString()}
                                                </td>
                                                <td class="py-4 px-4 text-sm text-black font-medium">
                                                    {tx.entity_name}
                                                </td>
                                                <td class="py-4 px-4 text-sm text-zinc-700">
                                                    {tx.source_id === params.id ? tx.destination_name : tx.source_name}
                                                </td>
                                                <td class="py-4 px-4 text-right text-sm text-zinc-700">
                                                    {Number(tx.rate).toFixed(2)}
                                                </td>
                                                <td class="py-4 px-4 text-right text-sm text-zinc-700">
                                                    {tx.quantity} {tx.unit}
                                                </td>
                                                <td class="py-4 px-4 text-right text-sm text-black font-medium">
...
const EmptyState = () => (
    <tr>
        <td colspan={11} class="text-center py-16 text-zinc-500">
