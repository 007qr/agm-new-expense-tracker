-- Drop the existing transaction_detail view (must drop before altering table)
DROP VIEW IF EXISTS "transaction_detail";

-- Drop the destination index
DROP INDEX IF EXISTS "transaction_destination_id_idx";

-- Remove destination_id column from transaction table
ALTER TABLE "transaction" DROP COLUMN IF EXISTS "destination_id";

-- Recreate transaction_detail view without destination fields
